...
def clip_recognition_dataset(
    ...
    assert os.path.exists(
        data_path
    ), "Please run data/parse_ego4d_json.py first. Will change this later"
...
def clip_forecasting_dataset(
    ...
    if g_pathmgr.isfile(data_path):
        ...
        for clip_uid, video_clips in annotations.items():
            ...
            if len(video_clips) <= 0 or not os.path.exists(video_path):
                continue

            # Extract forecasting annotations from video clips.
            for i in range(len(video_clips) - num_future_actions - num_input_actions):
